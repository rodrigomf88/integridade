...
class XsuaaService extends Service {
    #serviceUrl; // service URL against which unauthenticated requests may be sent
    endpoints = {
    ...
    constructor(credentials, serviceConfig) {
        ...
        this.#serviceUrl = this.determineServiceUrl();
    }

    validateCredentials() {
        const mandatoryProperties = ["xsappname", "clientid", "uaadomain", "url"];

        for (const p of mandatoryProperties) {
            if (!this.credentials[p]) {
                throw new InvalidCredentialsError(`${this.constructor.name} requires ${p} inside credentials.`);
            }
        }
    }

    /**
     * Determines the service URL from the credentials that can be used for unauthenticated requests to the service.
     */
    determineServiceUrl() {
    ...
    async getJwks(token, contextConfig) {
        ...
        const keyParts = { url: this.#serviceUrl, ...jwksParams };
    ...
    async fetchJwks(jwksParams, correlationId) {
        const jwksUrl = new URL(this.endpoints.jwks, this.#serviceUrl);
