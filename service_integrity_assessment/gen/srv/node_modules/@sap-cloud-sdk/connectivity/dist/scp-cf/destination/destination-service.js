...
async function fetchDestinations(destinationServiceUri, serviceToken, type, options) {
    ...
    return callDestinationEndpoint({ uri: targetUri, tenantId: getTenantFromTokens(serviceToken) }, headers)
        .then(response => {
...
async function fetchDestinationWithoutTokenRetrieval(destinationName, destinationServiceUri, serviceToken) {
    ...
    try {
        const response = await callDestinationEndpoint({ uri: targetUri, tenantId: getTenantFromTokens(serviceToken) }, { Authorization: `Bearer ${serviceToken}` });
...
async function fetchDestinationWithTokenRetrieval(destinationServiceUri, token, options) {
    ...
    authHeader = token.refreshToken
        ...
        : authHeader;
    return callDestinationEndpoint({ uri: targetUri, tenantId: getTenantFromTokens(token) }, authHeader, options)
        .then(response => {
